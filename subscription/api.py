...
def get_like(request, sub_id: int):
    ...
    try:
        ...
        if request.user.id != None:
            ...
            user = UserSubscription.objects.filter(user_subscription_id=subscription.id, user_id=request.user.id)
...
def post_like(request, data: schema.LikePostSchema):
...
def delete_like(request, data: schema.LikeDeleteSchema):
    ...
    sub_id = data.sub_id
    subscription = Subscription.objects.get(sub_id=sub_id)
    user_subcription = UserSubscription.objects.get(user_id=request.user.id, user_subscription_id=subscription.id)
